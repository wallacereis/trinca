using Acr.UserDialogs;
using ChurrasTrincaAPP.Interfaces;
using ChurrasTrincaAPP.Models;
using ChurrasTrincaAPP.ViewModels.Base;
using ChurrasTrincaAPP.ViewModels.Modal;
using System;
using System.Collections.Generic;
using System.Text;
using System.Threading.Tasks;
using Xamarin.CommunityToolkit.ObjectModel;
using Xamarin.Forms;

namespace ChurrasTrincaAPP.ViewModels.Cadastros
{
    sealed class RegistrarParticipanteViewModel : BasePageViewModel
    {
        #region [Properties]
        /*---------------------- ValorPago Properties ----------------------*/
        private decimal _valorPago;
        public decimal ValorPago
        {
            get { return _valorPago; }
            set { SetProperty(ref _valorPago, value); }
        }
        private Credential _credential;
        private BBQ _bbq;

        private readonly IFlurlAPI<Participant> _flurlAPI;
        private readonly IDialogService _dialogService;

        public IAsyncCommand ConfirmCommand { get; }
        public Participant ParticipantModel { get; set; }

        #endregion

        public RegistrarParticipanteViewModel()
        {
            _flurlAPI = DependencyService.Get<IFlurlAPI<Participant>>();
            _dialogService = DependencyService.Get<IDialogService>();

            ConfirmCommand = new AsyncCommand(ExecuteConfirmCommandAsync, allowsMultipleExecutions: false);

            ParticipantModel = new Participant
            {
                name = string.Empty,
                confirmed = false,
                value_paid = 0
            };
        }

        public override async Task Initialize(params object[] args)
        {
            await base.Initialize(args);
            //*------------- Load Parameters -------------*/
            _credential = (Credential)args[0];
            _bbq = (BBQ)args[1];
        }

        private async Task ExecuteConfirmCommandAsync()
        {
            if (!InternetConnectionActive())
            {
                await NavigationService.Navigate<SemConexaoViewModel>();
                return;
            }
            if (!ValidaFormulario())
            {
                //Toast Messages
                _dialogService.ShowToast("Por favor, informe o nome do Participante!");
                return;
            }
            await RegisterParticipantAsync();
        }

        private async Task RegisterParticipantAsync()
        {
            if (!IsBusy)
            {
                Exception Error = null;
                try
                {
                    IsBusy = true;
                    //Verificar se o Usuário está cadastrado via Backend
                    using (UserDialogs.Instance.Loading("Registrando...", null, null, true, MaskType.Gradient))
                    {
                        Participant objParticipant = new Participant
                        {
                            bbq_id = _bbq.id,
                            name = ParticipantModel.name,
                            confirmed = ParticipantModel.confirmed,
                            value_paid = ValorPago
                        };
                        var result = await _flurlAPI.AddItemAsync("participant", objParticipant, _credential.Token);
                    }
                }
                catch (Exception ex)
                {
                    IsBusy = false;
                    Error = ex;
                }
                finally
                {
                    IsBusy = false;
                    await NavigationService.Navigate<MensagemViewModel>(_credential, _bbq, "Participante registrado com sucesso!");
                }
                if (Error is not null)
                {
                    IsBusy = false;
                    await DisplayAlert("Atenção!", "Ocorreu algo inesperado!" + Environment.NewLine + "Por favor, tente novamente!", "OK");
                }
            }
        }

        private bool ValidaFormulario()
        {
            if (String.IsNullOrWhiteSpace(ParticipantModel.name.Trim()))
                return false;
            return true;
        }
    }
}
